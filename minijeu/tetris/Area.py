...
class Area:
    ...
    def draw(self, shape, matrix, screen):
        ...
        solid = pygame.image.load("minijeu/tetris/outside.png").convert()
    ...
    def print_game_info(self, screen):
        ...
        # render text
        label_1 = myfont.render("Lignes", 1, font_color)
        ...
        score_2_rect.center = (250, (self.height / 2)-50)

        score = myfont.render(str(self.score), 1, font_color)
        score_rect = score.get_rect()
        highscorefi = open("minijeu/tetris/highscore", "r")
        highscore = highscorefi.read()
        highscorefi.close()
        score_rect.center = (250, self.height / 2 -30)
        highscore_2 = myfont.render("Highscore:", True, font_color)
        highscore_2_rect = highscore_2.get_rect()
        highscore_2_rect.center = (250, (self.height / 2)+40)
        highscore = myfont.render(str(highscore), True, font_color)
        highscore_rect = highscore.get_rect()
        highscore_rect.center = (250, (self.height / 2)+70)
        screen.blit(label_1, label_1_rect)
        screen.blit(lines, lines_rect)
        screen.blit(highscore_2, highscore_2_rect)
